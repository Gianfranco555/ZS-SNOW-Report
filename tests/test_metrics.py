...
def test_kpis(metrics_results: dict):
    ...
    assert kpis["avg_ttr_hours"] == 20  # Rounded from 20.26


def test_kpis_open_states(metrics_results: dict):
    """Tests the open-by-state KPI for sorting and values."""
    open_by_state = metrics_results["kpis"]["open_by_state"]
    # Expected: sorted by count (desc), then state (asc)
    # New: 2, In Progress: 1, On Hold: 1
    # Expected order: New, In Progress, On Hold
    expected = {"New": 2, "In Progress": 1, "On Hold": 1}
    assert open_by_state == expected
    assert list(open_by_state.keys()) == ["New", "In Progress", "On Hold"]


def test_kpis_total_open_tickets(metrics_results: dict):
    """Tests the total_open_tickets KPI."""
    kpis = metrics_results["kpis"]
    assert kpis["total_open_tickets"] == 4  # 2 + 1 + 1
...
def test_table_open_by_state(metrics_results: dict):
    ...
    # The table itself is sorted alphabetically by state for presentation.
    expected = [
...
def test_empty_input():
    ...
    assert metrics["kpis"]["total_open_tickets"] == 0
    assert metrics["kpis"]["open_by_state"] == {
        "New": 0,
        "In Progress": 0,
        "On Hold": 0,
    }
